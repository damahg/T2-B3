...
public class PrincipalCuentaVersion2 {
	...
	public static void main(String[] args) {
		...
		//En esta versión, si se introduce un saldo negativo, vuelve a solicitarlo,
		//intentando crear el objeto cuenta hasta que sea correcto
		
		saldoInicial = PrincipalCuentaVersion2.pedirSaldoInicial();
		cuenta = crearCuenta();
		do {
			// Muestra el menú y solicita la opción
			opcion = PrincipalCuentaVersion2.menu();

			// Trata cada una de las opciones del menú
			salir = PrincipalCuentaVersion2.tratarMenu(opcion, cuenta);
		} while (salir == 'n');
	...
	private static Cuenta crearCuenta() {
		...
		do {
			try {
				saldo=solicitarDouble("Introduce el saldo inicial de la cuenta");
